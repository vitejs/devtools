...
This error is logged when a general RPC error occurs outside of a specific function invocation. It is triggered by the `onGeneralError` hook passed via `rpcOptions` to `createRpcServer` inside `createWsServer()`.
...
- [`packages/core/src/node/ws.ts`](https://github.com/vitejs/devtools/blob/main/packages/core/src/node/ws.ts) — `createWsServer()` registers the `onGeneralError` hook on the RPC server; the hook logs `DTK0012` for transport-level failures (serialization, WebSocket protocol, resolver pipeline).
- [`packages/core/src/node/diagnostics.ts`](https://github.com/vitejs/devtools/blob/main/packages/core/src/node/diagnostics.ts) — defines `DTK0012`.
