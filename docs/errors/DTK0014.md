...
- [`packages/core/src/node/context.ts`](https://github.com/vitejs/devtools/blob/main/packages/core/src/node/context.ts) — `createDevToolsContext()` wraps each plugin's `devtools.setup()` invocation; throws `DTK0014` (with the original error as `cause`) when the hook fails.
- [`packages/core/src/node/diagnostics.ts`](https://github.com/vitejs/devtools/blob/main/packages/core/src/node/diagnostics.ts) — defines `DTK0014`.
