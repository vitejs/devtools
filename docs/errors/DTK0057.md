...
- [`packages/kit/src/node/host-commands.ts`](https://github.com/vitejs/devtools/blob/main/packages/kit/src/node/host-commands.ts) — `execute` and the `update` closure both throw `DTK0057` when the command id can't be resolved via `findCommand` / `commands.get`.
