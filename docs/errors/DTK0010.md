...
- [`packages/core/src/node/cli-commands.ts`](https://github.com/vitejs/devtools/blob/main/packages/core/src/node/cli-commands.ts) — `build()` logs this after the static build command completes, flagging the feature's experimental status.
- [`packages/core/src/node/diagnostics.ts`](https://github.com/vitejs/devtools/blob/main/packages/core/src/node/diagnostics.ts) — defines `DTK0010` (level: `warn`).
