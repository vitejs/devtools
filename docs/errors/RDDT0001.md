...
- [`packages/rolldown/src/node/rpc/utils.ts`](https://github.com/vitejs/devtools/blob/main/packages/rolldown/src/node/rpc/utils.ts) — the Rolldown DevTools plugin probes both the project `cwd` and `process.cwd()` for `node_modules/.rolldown/`; logs `RDDT0001` when neither location exists.
- [`packages/rolldown/src/node/diagnostics.ts`](https://github.com/vitejs/devtools/blob/main/packages/rolldown/src/node/diagnostics.ts) — defines `RDDT0001` (level: `warn`).
