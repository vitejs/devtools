...
- [`packages/core/src/node/rpc/public/open-in-finder.ts`](https://github.com/vitejs/devtools/blob/main/packages/core/src/node/rpc/public/open-in-finder.ts) — the `vite:core:open-in-finder` RPC handler validates the resolved path; throws `DTK0029` when it escapes `context.workspaceRoot`.
- [`packages/core/src/node/diagnostics.ts`](https://github.com/vitejs/devtools/blob/main/packages/core/src/node/diagnostics.ts) — defines `DTK0029`.
