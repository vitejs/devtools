...
The `DevToolsViewHost.hostStatic()` method was called while Vite is running in `serve` (dev) mode, but the `viteServer` instance on the DevTools context is not available. In dev mode, the view host needs the Vite dev server to register its middleware for serving static files. This can happen if `hostStatic()` is called before the Vite server has been fully initialized and attached to the context.
