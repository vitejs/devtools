...
- [`packages/core/src/node/diagnostics.ts`](https://github.com/vitejs/devtools/blob/main/packages/core/src/node/diagnostics.ts) — defines `DTK0023`. Reserved for the dev-mode guard around `DevToolsViewHost.hostStatic()` requiring a Vite server on the context.
