...
This error is thrown by the RPC resolver in `createWsServer()` when an untrusted WebSocket client attempts to call a protected RPC method. The auth-handshake methods are exempt and callable by any client: names starting with `devframe:anonymous:` (e.g. `devframe:anonymous:auth`) plus `devframe:auth:exchange`. Every other method requires the client to be trusted.
...
2. **One-time code exchange** — an untrusted browser is shown a one-time code in the terminal, along with a `?devframe_otp=` magic link. Entering the code (or opening the link) exchanges it via `devframe:auth:exchange` for a node-issued bearer token; the browser persists that token and re-presents it (`devframe:anonymous:auth`) on reconnect.
...
- [`packages/core/src/node/ws.ts`](https://github.com/vitejs/devtools/blob/main/packages/core/src/node/ws.ts) — the WebSocket RPC resolver in `createWsServer()` throws `DTK0013` when an untrusted client invokes a method outside the pre-auth allowlist (the `devframe:anonymous:` prefix and `devframe:auth:exchange`).
