...
- [`packages/core/src/node/rpc/internal/docks-on-launch.ts`](https://github.com/vitejs/devtools/blob/main/packages/core/src/node/rpc/internal/docks-on-launch.ts) — the `devtoolskit:internal:docks:on-launch` handler looks up the dock id in `context.docks.values()`; throws `DTK0030` when no entry matches.
- [`packages/core/src/node/diagnostics.ts`](https://github.com/vitejs/devtools/blob/main/packages/core/src/node/diagnostics.ts) — defines `DTK0030`.
