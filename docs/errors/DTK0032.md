...
- [`packages/core/src/node/rpc/internal/docks-on-launch.ts`](https://github.com/vitejs/devtools/blob/main/packages/core/src/node/rpc/internal/docks-on-launch.ts) — the `devtoolskit:internal:docks:on-launch` handler invokes the launcher's `onLaunch()` callback; logs `DTK0032` (with the error as `cause`) when the callback throws and sets the entry's launcher status to `'error'`.
- [`packages/core/src/node/diagnostics.ts`](https://github.com/vitejs/devtools/blob/main/packages/core/src/node/diagnostics.ts) — defines `DTK0032`.
