...
- [`packages/core/src/node/rpc/public/open-in-editor.ts`](https://github.com/vitejs/devtools/blob/main/packages/core/src/node/rpc/public/open-in-editor.ts) — the `vite:core:open-in-editor` RPC handler validates the resolved path; throws `DTK0028` when it escapes `context.workspaceRoot`.
- [`packages/core/src/node/diagnostics.ts`](https://github.com/vitejs/devtools/blob/main/packages/core/src/node/diagnostics.ts) — defines `DTK0028`.
