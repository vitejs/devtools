...
DevTools Kit is the **Vite-specific superset** of [DevFrame](https://devtools.vite.dev/devframe/guide/) — the framework-neutral foundation. Everything in DevFrame (RPC, hosts, docks, shared state, agents) is available through Kit, plus Vite-specific glue: the `devtools` hook on Vite plugins, Vite dev-server integration, and the Vite DevTools client. If you're building a tool that doesn't need Vite at all, start from [DevFrame](https://devtools.vite.dev/devframe/guide/) directly.
