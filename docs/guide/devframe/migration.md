# Migrating an inspector to devframe
...
WebSocket RPC + static build), devframe absorbs the re-implementation
of that stack. Your inspector becomes a single `DevtoolDefinition`
plus an author-provided SPA.

## Mapping table

| Today's concept                        | Devframe equivalent                                  |
|----------------------------------------|-----------------------------------------------------|
...
Adopt `devframeVite(devtool)` for the plain Vite case, or
`toKitPlugin(devtool)` if embedding in Vite DevTools.

## Common gotchas

- **Auth tokens** — the default devframe auth flow mirrors Vite
...
devframe-owned DF codes (see `docs/errors/DF\**`). Vite-specific codes
