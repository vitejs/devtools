...
export function useGraphPathSelector<T extends { id: string }>(options: {
  ...
  const fuse = ref<ComputedRefWithControl<Fuse<T>>>()
...
export function useGraphPathManager<T extends { id: string, imports: unknown[] }>(
  options: {
    onToggle: (visible: boolean) => void
    dataMap: ComputedRef<Map<string, { imports?: unknown[] }>>
    list: ComputedRef<T[]>
    importIdKey: string
  },
) {
  ...
  const normalizedGraph = computed(() => {
    ...
    const getImportIds = (id: string) => {
      return dataMap.value.get(id)?.imports?.map(i => `${(i as Record<string, unknown>)[importIdKey]}`) || []
    }
