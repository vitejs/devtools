...
<template>
  <Tooltip>
    <div class="flex gap-1 items-center text-sm pl1 pr2 rounded border-l-3 border" :style>
      <div v-if="parsed.namespace" class="text-xs op-fade">
        {{ parsed.namespace }}:
      </div>
      <div class="rounded-full font-mono">
        {{ parsed.value }}
      </div>
    </div>
    <template #popper>
