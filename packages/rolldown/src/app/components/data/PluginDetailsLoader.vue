...
<template>
  ...
  <div v-else-if="state?.calls?.length" class="relative h-full w-full">
    <DisplayCloseButton
    ...
    >
      ...
      <div class="text-xs font-mono flex items-center gap-3 ml2">
        ...
        <DisplayDuration
          :duration="hookLoadDuration" class="flex gap-1 items-center"
          :title="`Load hooks cost: ${formatDuration(hookLoadDuration, true)}`"
        >
          <span class="i-ph-upload-simple-duotone inline-block" />
        </DisplayDuration>
        <DisplayDuration
          :duration="hookTransformDuration" class="flex gap-1 items-center"
        ...
        <DisplayDuration
          :duration="totalDuration" class="flex gap-1 items-center"
  ...
  <DisplayGraphHoverView :hover-x="mouse.x" :hover-y="mouse.y">
    ...
    >
      <div v-if="nodeHover.meta?.title" class="flex items-center gap2">
        {{ nodeHover.meta.title }}
        <DisplayDuration :duration="nodeHover.meta.duration" />
      </div>
      <template v-else>
        <div v-if="nodeHover.meta?.module" class="flex font-semibold font-mono text-base mb2">
          <DisplayModuleId :id="nodeHover.meta.module" :session="session" :link="false" />
        </div>
        <div v-if="nodeHover.meta?.module" class="border-t border-base pt2 flex flex-col gap-1.5 min-w-48">
