...
export async function buildSpa(d: DevtoolDefinition, options: BuildSpaOptions): Promise<void> {
  ...
  if (d.setupBrowser) {
    console.log(c.yellow`[takubox] buildSpa: "${d.id}" declares setupBrowser but in-browser bundling is not yet implemented. Ship a separate client entry that registers the handlers.`)
  }

  console.log(c.green`[takubox] spa built for "${d.id}" (loader: ${spaLoader.mode}) -> ${outDir}`)
}
