...
export async function buildStatic(d: DevtoolDefinition, options: BuildStaticOptions): Promise<void> {
  ...
  if (!distDir)
    throw new Error(`[takubox] buildStatic: no distDir for "${d.id}". Set \`cli.distDir\` on the definition or pass it as an option.`)
  ...
  console.log(c.cyan`[takubox] copying SPA from ${distDir} -> ${outDir}`)
  ...
  console.log(c.cyan`[takubox] writing RPC dump to ${resolve(devToolsRoot, DEVTOOLS_RPC_DUMP_MANIFEST_FILENAME)}`)
  ...
  console.log(c.green`[takubox] built "${d.id}" -> ${outDir}`)
}
