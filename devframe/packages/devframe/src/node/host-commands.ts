import type { DevToolsCommandHandle, DevToolsCommandsHost as DevToolsCommandsHostType, DevToolsNodeContext, DevToolsServerCommandEntry, DevToolsServerCommandInput } from 'devframe/types'
...
export class DevToolsCommandsHost implements DevToolsCommandsHostType {
  ...
  register(command: DevToolsServerCommandInput): DevToolsCommandHandle {
    if (this.commands.has(command.id)) {
      throw logger.DF0009({ id: command.id }).throw()
    }
    ...
    return {
      ...
      update: (patch: Partial<Omit<DevToolsServerCommandInput, 'id'>>) => {
        if ('id' in patch) {
          throw logger.DF0010().throw()
        }
        const existing = this.commands.get(command.id)
        if (!existing) {
          throw logger.DF0011({ id: command.id }).throw()
        }
