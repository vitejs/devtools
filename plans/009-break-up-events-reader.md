# Plan 009: Break up the `RolldownEventsReader` god class
...
>
> **Prerequisite gate**: plan 008 (characterization tests for events-reader/log-cache)
...
- **Effort**: L
- **Risk**: MED
- **Depends on**: plans/008 (characterization net)
...
concurrency issue characterized by the existing test and the payload issue in plan 006).
...
orchestrator — **without changing observable behavior** (the plan 008 tests are the gate).
...
- After plan 008, `__tests__/events-reader.test.ts` (extended) + `__tests__/log-cache.test.ts`
...
absent or failing on the untouched tree, STOP (plan 008 is the prerequisite).
...
named collaborators. The plan 008 characterization tests must still pass unchanged.

**Verify**: `pnpm -C packages/rolldown build` → exit 0; full suite green.

## Test plan

- Plan 008's characterization tests are the regression gate (must pass unchanged at every commit).
...
ALL must hold:
- [ ] Plan 008 tests pass unchanged throughout (no edits to them to accommodate the refactor)
...
Stop and report if:
- Plan 008's tests are absent or red on the untouched tree.
...
- This unblocks safer perf work (plan 006 and the concurrency hazards) inside a smaller surface.
