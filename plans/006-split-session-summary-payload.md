# Plan 006: Split the Rolldown `get-session-summary` payload
...
- **Effort**: M
- **Risk**: MED
- **Depends on**: none (but coordinate with 004/008 which touch the same package)
- **Category**: perf
...
(no `build_metrics`), given a stub reader. Reuse the fake-reader approach from plan 004 /
plan 008's fixtures if available.
